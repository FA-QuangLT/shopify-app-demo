...
class AuthApi
{
    ...
    function verifyRequest(array $data) : bool
    {
        ...
        if(array_key_exists('hmac', $tmp)) {
            ...
            $match       = $tmp['hmac'];
            $calculated  = hash_hmac('sha256', $queryString, config('shopify.secret_key'));
    ...
    function getAccessToken(string $shop, string $code) : array
    {
        ...
        $client = new Client();
        try{
            $response = $client->request('POST', "https://{$shop}.myshopify.com/admin/oauth/access_token.json",
                [
                    'headers' => [
     ...
     * @return array
     */
    function resigterWebhooks(string $action, $address) : array
    {
        $shop = session('store')['name'];
        $token = session('store')['access_token'];
        $client = new Client();
        try{
            $response = $client->request('POST', "https://{$shop}.myshopify.com/admin/api/2020-01/webhooks.json",
                [
                    'headers' => [
                        'Content-Type' => 'application/json',
    ...
    function callApi (string $method, string $url, array $data = []) {
        $shop = session('store')['name'];
        $token = session('store')['access_token'];
        $client = new Client();
        try{
            ...
            $response = $client->request($method, "https://{$shop}.myshopify.com/". $url, $options);
            return ['status' => true, 'data' => json_decode($response->getBody()->getContents(), true)];
        } catch (\Exception $exception) {
            return ['status' => false, 'message' => $exception->getMessage()];
        }
